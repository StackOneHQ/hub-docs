...
You can use the Client ID and Client Secret to link your Docebo account to StackOne.
  <Steps>
    <Step title="Link Docebo Account">
      In the StackOne dashboard, navigate to the `Accounts` page.

      Click `Link Account` and select Docebo.

      In the modal enter the `Client ID`, `Client Secret`, `Subdomain`, `Username` and `Password` from the previous steps.
