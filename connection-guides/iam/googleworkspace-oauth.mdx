...
If you've been directed to StackOne to integrate with Google Workspace, the following steps will help you understand the process and configure a successful integration.
...
Each connection will be associated with a single Google Workspace domain. If you wish to connect multiple domains, you'll need to create a separate connection for each domain.
...
<Steps>
    ...
    <Step title="Copy Your Domain">
        Copy the **Domain** you wish to connect from the displayed list. Save this for use in later steps.
...
<Steps>
    <Step title="Go to Your Google Cloud Console">
        Navigate to your Google Cloud Console at https://console.cloud.google.com/
    ...
    <Step title="Locate the Admin SDK API">
        In the search bar, enter *"admin sdk"*. Click the **Admin SDK API** result.
...
<Steps>
    ...
    <Step title="Navigate to Admin SDK API Credentials">
        On the Admin SDK API **Manage** page, scroll down and click **Credentials**.
    ...
    <Step title="Copy Application Credentials">
        The new application's **Client ID** and **Client Secret** will be displayed. **Copy** these values and store them securely for use in the next step.
...
Google requires configuring a consent screen that will be displayed when granting application access to your account.
...
<Steps>
    ...
    <Step title="Select Required Application Scopes">
        Click the **Add or Remove Scopes** button. A popout window titled *Update selected scopes* will appear on the right.
        ...
        Enable the following scopes required for full functionality of this integration:
        ...
        You can quickly locate each scope by copying and pasting it into the **Filter** bar.
        ...
        After enabling all five scopes, click the **Update** button to proceed.
    ...
    <Step title="Complete Scope Selection">
        Verify that each of the required scopes is listed under *Your sensitive scopes*.
...
<Steps>
  ...
  <Step title="Grant OAuth Application Consent">
    ...
    After logging in, you'll see the OAuth consent screen you configured in a [previous step](#configure-oauth-consent).
