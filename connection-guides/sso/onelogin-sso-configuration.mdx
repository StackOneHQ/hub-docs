...
- **Docebo**: Use `{user.email}` or `{user.username}` for the user identifier
- **Cornerstone OnDemand**: May require `{user.employee_id}` or custom identifier
- **SAP SuccessFactors**: Typically uses `{user.username}` or email-based identifiers
- **Workday Learning**: Often requires `{user.employee_id}` or custom identifier
- **360Learning**: Usually accepts `{user.email}` or `{user.username}`
