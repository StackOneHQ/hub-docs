...
## Create a subscription and set up an external event

<Steps>
  <Step title="Create a subscription">
    Go to the _Event Notification Subscription_ page. Under the _Subscription_ tab, add a subscription using your `Subscription Id` and `Client Id`. Note that your `clientId` should match the one uploaded with your vendor's name in the previous step.
    <Frame>
...
<Steps>
  <Step title="Go to Job Requisition">
    ...
    Select the following values from the dropdown menus in the assessment:
